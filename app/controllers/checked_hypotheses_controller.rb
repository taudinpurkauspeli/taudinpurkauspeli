class CheckedHypothesesController < ApplicationController
      ...
      if(exHyp.user_meets_requirements(current_user))
        respond_to do |format|
          if @checked_hypothesis.save
            format.html { redirect_to hypotheses_url(:layout => false), notice: get_explanation(@checked_hypothesis)}
          else
            format.html { redirect_to hypotheses_url(:layout => false), notice: "Hypoteesin poisto epäonnistui" }
          end
        end
      else
        redirect_to hypotheses_url(:layout => false), notice: "Sinulla ei ole vielä tarpeeksi tietoa voidaksesi poissulkea työhypoteesin."
      end
    else
      redirect_to hypotheses_url(:layout => false), notice: "Sinulla ei ole vielä tarpeeksi tietoa voidaksesi poissulkea työhypoteesin."
    end
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to hypotheses_url(:layout => false), notice: 'Työhypoteesi palautettu mahdollisten työhypoteesien listaan.'}
