...
RSpec.describe OptionsController, :type => :controller do
	...
	let!(:multichoice){FactoryGirl.create(:multichoice, subtask:subtask)}
	...
	let(:valid_attributes) {{content: "4 jalkaa", explanation: "Normaalilla lehmällä on kaksi etujalkaa ja kaksi takajalkaa",
		is_correct_answer: "required", multichoice_id: multichoice.id
	}}

	let(:invalid_attributes) {{content: nil, explanation: nil, multichoice_id: multichoice.id }}
	...
	describe "DELETE destroy" do
		...
		it "redirects to the multichoice edit" do
			option = Option.create! valid_attributes
			delete :destroy, {:id => option.to_param}, valid_session
			expect(response).to redirect_to(edit_multichoice_path(multichoice.id, :layout => true))
		end
	end
end
